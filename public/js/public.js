

function renderTickets(tickets = 0) {
  for (let i = 0; i < tickets.length; i++) {
    if (i>= 4) break;
    const ticket = tickets[i];
    if (!ticket) continue;


    const lblTicket = document.querySelector(`#lbl-ticket-0${i+1}`);
    const lblDesk = document.querySelector(`#lbl-desk-0${i+1}`);

    lblTicket.innerHTML = `Ticket ${ticket.number}`;
    lblDesk.innerHTML = ticket.handleAtDesk;

  }
}

async function loadCurrentTickets() {
  const working = await fetch('/api/ticket/working-on').then(resp => resp.json()); 
  renderTickets(working)
  
}



function connectToWebSockets() {

  const socket = new WebSocket( 'ws://localhost:3000/ws' );

  socket.onmessage = ( event ) => {
    const numberTickets = JSON.parse(event.data);
    if (numberTickets.type !== 'on-working-change') return;
    renderTickets(numberTickets.payload)
  };

  socket.onclose = ( event ) => {
    console.log( 'Connection closed' );
    setTimeout( () => {
      console.log( 'retrying to connect' );
      connectToWebSockets();
    }, 1500 );

  };

  socket.onopen = ( event ) => {
    console.log( 'Connected' );
  };




}

connectToWebSockets()
loadCurrentTickets()